...
function(input, output, session) {
  ...
  alpha <- reactiveValues(samples = vector(), bool = c(F, F))
  ...
  # triggering events above trickle down to trigger event below
  observeEvent(input$group1, {
    i <- which(colnames(alpha$df) == input$group1)
    # update the data to be numerical column such that categorical == group 1
    alpha$data1 <- unlist(alpha$df[,i])
  })
  observeEvent(input$group2, {
    i <- which(colnames(alpha$df) == input$group2)
    # update the data to be numerical column such that categorical == group 1
    alpha$data2 <- unlist(alpha$df[,i])
  })
  
  observeEvent(c(alpha$data1, alpha$data2), {
    alpha$data_diff <- alpha$data1 - alpha$data2
    alpha$mu_diff <- mean(alpha$data_diff)
    alpha$sd_diff <- sd(alpha$data_diff)
  })
  ...
  observeEvent(input$generate, {
    ...
    new_samples <- replicate(
      n, Paired_T_Test(alpha$data1, alpha$data2)
    )
    alpha$samples <- c(alpha$samples, new_samples)
    alpha$bool[1] = length(alpha$samples) > 0
  })
  ...
  output$og_info0 <- renderUI({
    HTML(paste("Count:", length(alpha$data1)))
  })
  ...
  output$og_info3 <- renderUI({
    str0 <- paste("Mean Difference:", round(alpha$mu_diff, 3))
    str1 <- paste("SD Difference:", round(alpha$sd_diff, 4))
    HTML(paste(
      str0,
      str1,
      sep = "<br>"))
  ...
  output$results <- renderText({
    if (alpha$bool[1]) {
      ...
      alpha$sd <- signif(sd(alpha$samples), 4)
      
      if (alpha$limit != 0) {
        
        if (input$count_type == "greater than") {
        ...
        } else if (input$count_type == "less than") {
          samples_df <- samples_df %>%
            mutate(
              int_color = ifelse(
                samples <= alpha$limit,
          ...
          new_limit = abs(alpha$limit)
          samples_df <- samples_df %>%
            mutate(
              int_color = ifelse(
                samples >= new_limit | samples <= -new_limit,
                c_outer, c_inner))
        }
        
        alpha$n_outbounds <- sum(samples_df['int_color'] == c_outer)
        
      } else {
        samples_df <- samples_df %>%
          mutate(int_color = c_inner)
      }
      ...
      total <- length(alpha$samples)
      if (alpha$bool[2]) {
        paste("Estimated p-value =",alpha$n_outbounds,
              "/",total,"=",signif(alpha$n_outbounds/total,4))  
      }
  ...
  output$distribution <- renderPlot({
    ...
    if (alpha$bool[1]) {
      ...
      if (input$plot_type == "Dotplot") {
        ...
        if (alpha$n_outbounds > 0 & is.numeric(alpha$limit)) {
          # Multicolored Plot
          ...
          if ("Show Selected Sample" %in% input$lines) {
            new_plot <- new_plot +
              geom_point(aes(
                x_vals, y_vals, color = c_inner, size = selected))
          } else {
            new_plot <- new_plot +
              geom_point(aes(
                x_vals, y_vals, color = c_inner))
          }
          new_plot <- new_plot +
            scale_y_continuous(
              limits = c(0, max(n_bin*0.75, max(new_df$y_vals)*1.01))) +
            scale_color_manual(values = c_inner)
        }
        ...
        if ("Overlay Normal Curve" %in% input$lines) {
          ...
          new_plot <- new_plot +
            stat_function(
              fun = function(x) dnorm(x, 0, overlay_sd) * 
                max(new_df$y_vals) / dnorm(0, 0, overlay_sd),
        ...
        if (alpha$n_outbounds > 0 & is.numeric(alpha$limit)) {
          ...
          # Monocolored Plot
          new_plot <- ggplot(alpha$samples_df) +
            geom_histogram(aes(x = samples), fill = c_inner,
                           bins = n_bin, color = c_select) +
            scale_color_manual(values = c_outer) +
            scale_fill_manual(values = c_inner)
        }
        if ("Show Selected Sample" %in% input$lines) {
      ...
      # if("Show Observed Line" %in% input$lines & alpha$bool[2]) {
      #   new_plot <- new_plot +
      #     geom_vline(xintercept = alpha$limit, size = 2, 
      #                alpha = 0.5, linetype = "longdash", color = c_outer)
      # }
      ...
      new_plot +
        scale_x_continuous(
          breaks = signif(c(-x_ticks, x_ticks[-1]), 2)) +
  ...
  output$data_visuals <- renderPlot({
    ...
    # Title
    alpha$og_diff <- signif(alpha$og_mean1 - alpha$og_mean2, 4)
