...
#############################
# Two Mean Diff Base Source #
#############################

library(openintro)
library(ggplot2)
classdata$lecture <- as.character(classdata$lecture)

# presets
Presets <- list("Custom","Upload","classdata","mtcars","iris","chickwts","births14")
DataPresets <- list(classdata, mtcars, iris, chickwts, as.data.frame(births14))
...
TwoMeanDiffCI <- function(x, y) {
  # sample with replacement from x
  xbar <- mean(sample(x, replace = T))
  
  # sample with replacement from y
  ybar <- mean(sample(y, replace = T))
  
  # return the difference of the respective means
  return(ybar - xbar)
}

TwoMeanDiffTest <- function(x,y) {
  # takes 2 vectors of size > 0
  # concatenates them into 1 vector
  data <- c(x,y)
  # samples without replacement (randomizing them)
  data <- sample(data, length(data), replace = FALSE)
  # the first nx values are the x values where nx is length(x)
  n <- length(x)
  x <- data[1:n]
  # the rest (ny) are y values
  y <- data[(n+1):length(data)]
  # take mean, y - x, return
